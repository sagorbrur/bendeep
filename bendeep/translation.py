...
def training(data_path, hidden_size=256, iteration=75000):
  ...
  evaluateRandomly(data_path, encoder1, attn_decoder1)
...
def bn2en(input_lang, output_lang, encoder_model, decoder_model, input_sentence, hidden_size=256):
  ...
  try:
    ...
    output, attn = evaluate(encoder, decoder, input_lang, output_lang, input_sentence)
    output = output[:-1]
    output = " ".join(output)
